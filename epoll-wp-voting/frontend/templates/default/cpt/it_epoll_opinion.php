...
while ( have_posts() ) : the_post();
			...
			if($it_epoll_vote_end_date_time){
				...
				if($it_epoll_gen_days >= 0){
					if($it_epoll_gen_days > 0){
						if($it_epoll_gen_days > 1) $it_epoll_gen_days_text = __(' Days','epoll-wp-voting'); else $it_epoll_gen_days_text = __(' Day','epoll-wp-voting');
						$it_epoll_vote_end_date_time_left = $it_epoll_gen_days.$it_epoll_gen_days_text;
	
					}else if($it_epoll_gen_hours > 1){
						if($it_epoll_gen_hours > 1) $it_epoll_gen_hours_text = __(' Hours','epoll-wp-voting'); else $it_epoll_gen_hours_text = __(' Hour','epoll-wp-voting');
						$it_epoll_vote_end_date_time_left = $it_epoll_gen_hours.$it_epoll_gen_hours_text;
					}else{
						$it_epoll_gen_mins = $it_epoll_gen_hours/60;
						if($it_epoll_gen_mins > 1) $it_epoll_gen_mins_text = __(' Minutes','epoll-wp-voting'); else $it_epoll_gen_mins_text = __(' Minute','epoll-wp-voting');
		...
		?>
			<style type="text/css">
			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .ec_poll_border_top {
					border-color:<?php echo esc_attr($it_epoll_poll_color_primary,'epoll-wp-voting');?>;
			}
			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?>  .eg_button, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .epoll_powered_by_Link{
				background:<?php echo esc_attr($it_epoll_poll_color_primary,'epoll-wp-voting');?>;
				color:<?php echo esc_attr($it_epoll_poll_button_text_color,'epoll-wp-voting');?>;
				border-color:<?php echo esc_attr($it_epoll_poll_color_primary,'epoll-wp-voting');?>;
			}
			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .epoll_sc_share_container li a{
				color:<?php echo esc_attr($it_epoll_poll_color_primary,'epoll-wp-voting');?>;
			}
			 <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .eg_badge,.eg_button_secondary, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .eg_button:hover, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .eg_button:focus, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .eg_button:active, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .epoll_poll_option_wrapper ul.epoll_poll_options li .it_epoll_poll_opt, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .epoll_poll_option_wrapper ul.epoll_poll_options li .it_epoll_opt_radio_wrap .it_epoll_otp_result_right{
				color:<?php echo esc_attr($it_epoll_poll_color_primary,'epoll-wp-voting');?>;
				background:<?php echo esc_attr($it_epoll_poll_color_secondary,'epoll-wp-voting');?>;
				outline-color: <?php echo esc_attr($it_epoll_poll_color_mouseover,'epoll-wp-voting');?>;
				border-color: <?php echo esc_attr($it_epoll_poll_color_mouseover,'epoll-wp-voting');?>;
			}

			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_opt_radio_wrap [type="radio"]:checked + label, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_opt_radio_wrap [type="radio"]:not(:checked) + label {
				color: <?php echo esc_attr($it_epoll_poll_option_text_color,'epoll-wp-voting');?>;
			}

			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_opt_radio_wrap [type="radio"]:checked + label{
				color:<?php echo esc_attr($it_epoll_poll_color_primary,'epoll-wp-voting');?>;
			}
			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .epoll_badge_danger {
				background: #ff9b94;
				color: #a11f15;
				border: 0;
			}
			
			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .epoll_poll_option_wrapper ul.epoll_poll_options li .it_epoll_poll_opt:hover, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .epoll_poll_option_wrapper ul.epoll_poll_options li .it_epoll_poll_opt:active, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .epoll_poll_option_wrapper ul.epoll_poll_options li .it_epoll_poll_opt:focus{
				background:<?php echo esc_attr($it_epoll_poll_color_result_color,'epoll-wp-voting');?>;
			}

			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_otp_result_wrap, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_opt_radio_wrap [type="radio"]:checked + label:before, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_opt_radio_wrap [type="radio"]:not(:checked) + label:before{
				background: <?php echo esc_attr($it_epoll_poll_color_mouseover,'epoll-wp-voting');?>;
    			box-shadow: 0 0 2px <?php echo esc_attr($it_epoll_poll_color_mouseover,'epoll-wp-voting');?>;
			}
			
			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_opt_radio_wrap [type="radio"]:checked + label:before {
    			box-shadow: 0 0 2px <?php echo esc_attr($it_epoll_poll_color_primary,'epoll-wp-voting');?>;
			}

			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_otp_result_right{
				color:<?php echo esc_attr($it_epoll_poll_option_text_color,'epoll-wp-voting');?>;
			}
			<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_opt_radio_wrap [type="radio"]:checked + label:after, <?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> .it_epoll_opt_radio_wrap [type="radio"]:not(:checked) + label:after{
				background: <?php echo esc_attr($it_epoll_poll_color_primary,'epoll-wp-voting');?>;
			}
...
<div class="epoll_container" id="epoll_container_opinion">
				...
				<div class="epoll_card ec_epoll_extra_radius ec_epoll_extra_radius_card">
					...
					<div class="epoll_card_front_face ec_epoll_extra_radius ec_poll_border_top">
						...
						<div class="epoll_category_badges epoll_grids">
							...
							<div class="epoll_grid eg_grid_auto_left eg_text-left eg_pb-0 eg_pt-2">
								<?php if($it_epoll_poll_status == 'live'){?>
										<span class="eg_badge epoll_badge_danger"><i class="eicon eicon-wifi"></i> <?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_live_badge_text'),'epoll-wp-voting');?></span>
									<?php }elseif($it_epoll_poll_status == 'upcoming'){?>
										<span class="eg_badge"><i class="eicon eicon-rotation"></i> <?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_end_badge_text'),'epoll-wp-voting');?></span>
									<?php }else{?>
										<span class="eg_badge"><i class="eicon eicon-rotation"></i> <?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_upcoming_badge_text'),'epoll-wp-voting');?></span>
									<?php } ?>
							</div>
							<!-- End Category Name-->
							...
							<div class="epoll_grid eg_grid_auto_right eg_text-right eg_pb-0 eg_pt-2 eg_p_relative" id="it_epoll_opinion_share_btn">
								...
								<ul class="epoll_sc_share_container hide_epoll_sc_share_container">
									<?php if(get_option('it_epoll_settings_social_option_facebook')){?>
										<li>
											<a href="<?php echo esc_url('https://www.facebook.com/sharer/sharer.php?u='.get_the_permalink().'&t='.it_epoll_get_branding_sharer_text(),'epoll-wp-voting');?>" target="_blank"><i class="eicon eicon-facebook"></i> <?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_share_on_menu_text').' Facebook','epoll-wp-voting');?></a>
										</li>
									<?php }?>
									<?php if(get_option('it_epoll_settings_social_option_twitter')){?>
										<li>
											<a href="<?php echo esc_url('https://twitter.com/share?text='.get_the_title().' '.it_epoll_get_branding_sharer_text().'&url='.get_the_permalink(),'epoll-wp-voting');?>" target="_blank"><i class="eicon eicon-twitter"></i> <?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_share_on_menu_text').' Twitter','epoll-wp-voting');?></a>
										</li>
									<?php }?>
									<?php if(get_option('it_epoll_settings_social_option_whatsapp')){?>
										<li>
											<a href="<?php echo esc_url('https://wa.me/?text='.get_the_title().' You Can Vote Here : '.get_the_permalink().' '.it_epoll_get_branding_sharer_text(),'epoll-wp-voting');?>" target="_blank"><i class="eicon eicon-whatsapp"></i> <?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_share_on_menu_text').' WhatsApp','epoll-wp-voting');?></a>
										</li>
									<?php }?>
						...
						<p class="epoll_poll_sub_ttl epoll_poll_sub_single_ttl eg_text-left eg_py-0">
							<?php echo esc_html($it_epoll_poll_description,'epoll-wp-voting');?>
						</p>
			...
			<div class="epoll_poll_option_wrapper">
				<input type="hidden" name="it_epoll_poll_id" id="it_epoll_poll-id" value="<?php echo esc_attr($poll_id,'epoll-wp-voting');?>" required/>	
		...
		<?php }
			...
			?>
			<!-- EPOLL poll option-->
			<li class="eg_text-left epoll_poll_option_item" id="epoll_poll_option_id_<?php echo esc_attr($it_epoll_poll_option_id[$i],'epoll-wp-voting');?>">
				...
				if((!get_option('it_epoll_settings_hide_voting_result') && $it_epoll_result_visibility == 'public') || ($it_epoll_result_visibility == 'after_vote_end' && $it_epoll_poll_status == 'end')){?>
					...
					<div class="it_epoll_poll_opt_progressbar<?php if($epoll_hide_element) echo esc_attr(' epoll_hide_element','epoll-wp-voting');?>" data-count="<?php echo esc_html($it_epoll_poll_vote_percentage,'epoll-wp-voting');?>%">
					...
					<!-- Start Checkbox Action-->
						<div class="it_epoll_opt_radio_wrap">
							<div class="it_epoll_opt_radio_wrap_inner">
								...
								<input type="checkbox" class="it_epoll_opt_radio" id="epoll_poll_option_label_<?php echo esc_attr($it_epoll_poll_option_id[$i],'epoll-wp-voting');?>" value="<?php echo esc_attr($it_epoll_poll_option_id[$i],'epoll-wp-voting');?>" name="it_epoll_option_id[]" required <?php if(!$epoll_hide_element) echo esc_attr('disabled','epoll-wp-voting');?>/>
								<?php }else{?>
								
								<input type="radio" class="it_epoll_opt_radio" id="epoll_poll_option_label_<?php echo esc_attr($it_epoll_poll_option_id[$i],'epoll-wp-voting');?>" value="<?php echo esc_attr($it_epoll_poll_option_id[$i],'epoll-wp-voting');?>" name="it_epoll_option_id[]" required <?php if(!$epoll_hide_element) echo esc_attr('disabled','epoll-wp-voting');?>/>
								<?php }?>
									<!-- Start Checkbox Action title-->
								<label for="epoll_poll_option_label_<?php echo esc_attr($it_epoll_poll_option_id[$i],'epoll-wp-voting');?>" class="epoll_show_radio<?php if(!$epoll_hide_element) echo esc_attr(' epoll_hide_radio','epoll-wp-voting');?>"><span><?php echo esc_html($it_epoll_option_name,'epoll-wp-voting');?></span></label>
								<!-- end Checkbox Action title-->
							</div>
							<!-- Start ProgressBar Count -->
							<div class="it_epoll_otp_result_right eg_text-right"><?php echo esc_html($it_epoll_poll_vote_percentage,'epoll-wp-voting');?>%
								<span><?php echo esc_html($it_epoll_poll_vote_count_text,'epoll-wp-voting');?></span>
							</div>
	...
	<?php
		...
		<?php  if(!$epoll_hide_element){?>
			...
			jQuery('<?php echo esc_attr('#it_epoll_container_'.$poll_id,'epoll-wp-voting');?> #epoll_container_opinion').each(function(){
	...
	<?php if(get_option('it_epoll_settings_hcaptcha_voting')){
								...
								<div class="it_epoll_container_alert" id="it_epoll_hcaptcha">
									<div class="it_epoll_container_alert_inner">
										<h3><span class="it_epoll_container_alert_close"><i class="eicon eicon-left-arrow"></i> Back</span> <span><?php esc_attr_e('Validate that you are not a Robot','epoll-wp-voting');?></span></h3>
										<div class="h-captcha it_epoll_hcaptcha" data-size="normal" data-sitekey="<?php echo esc_attr($site_key,'epoll-wp-voting');?>"></div>
									</div>
								</div>
						...
						<div class="epoll_grids eg_border_t eg_border_solid eg_border_accent eg_pt-2 eg_pb-2">
							...
							<div class="epoll_grid eg_col eg_col-8  eg_text-left eg_p-0 eg_mobile_text_center">
								...
								<span class="epoll_category_count"><?php echo esc_attr(sprintf(it_epoll_poll_get_ttext('it_epoll_settings_time_left_text'),$it_epoll_vote_end_date_time_left),'epoll-wp-voting');?></span>
							...
							<button type="button" class="eg_button eg_button_big_x eg_button_secondary eg_mb-2_mobile eg_mt-1 eg_mr-2" id="epoll_opinion_show_result_button" style="<?php if(get_option('it_epoll_settings_hide_poll_result')) echo esc_attr('display:none;','epoll-wp-voting');?>"><i class="eicon eicon-pie-chart"></i> <?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_result_button_text'),'epoll-wp-voting');?></button>	
								
							<button type="button" class="eg_button eg_button_big_x eg_button_secondary eg_mb-2_mobile eg_mt-1 epoll_hide_element" id="epoll_opinion_hide_result_button" style="width:100%;"><i class="eicon eicon-left-arrow-1"></i>  <?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_result_back_button_text'),'epoll-wp-voting');?></button>	
							<?php }?>
							<button class="eg_button eg_button_big_x" type="submit" id="epoll_opinion_vote_button"><?php echo esc_attr(it_epoll_poll_get_ttext('it_epoll_settings_vote_button_text'),'epoll-wp-voting');?></button>
						...
						<?php do_action('it_epoll_opinion_ui_after_options',array('poll_id'=>$poll_id)); // add extra meta option here?>
							...
							<a href="<?php echo esc_url('https://wordpress.org/plugins/epoll-wp-voting/','epoll-wp-voting');?>" target="_blank" class="epoll_powered_by_Link">
							<i class="eicon eicon-info"></i> <?php echo esc_html( it_epoll_get_branding_text() ); ?>
							</a>
							<?php endif; ?>
						<!-- End Action Grids-->
						</div>
